function [u, J] = optCtrl(y, Uinit, S, R, N, W, A, B, C, D, x0, mode, u_min, u_max, dt, modelPars, critStruct)
...
%
%             Methods 1, 3 are model-based and use a built-in model (provide your specification below, function f) if N > 1
%             Methods 2, 4 use estimated state-space model described by (A, B, C, D, x0est)
%             Methods 3, 4 are model-free if N=1
%
...
%% Actor cost function
    function [J, gradJ] = costFncCtrl(U, y)
        ...
        if (mode == 1) || (mode == 2)       % MPC
            ...
            for k = 1:N
                J = J + Y(:,k)' * S * Y(:,k) + myU(:,k)' * R * myU(:,k);
            end
            
        elseif (mode == 3) || (mode == 4)   % RL/ADP (stacked Q-learning)
        ...
        elseif  (mode == 5) || (mode == 6)   % RL/ADP (N roll-outs of running cost)
